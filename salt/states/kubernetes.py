...
def deployment_present(
    ...
    **kwargs
    ...
    if deployment is None:
        ...
        changes = __obj_definition_diff(__translate_deployment_env(deployment), __translate_deployment_env(res))
...
def __obj_definition_diff(old, new, ignore_keys=[]):
    ...
    """
    ignore_keys = ignore_keys + ["kubernetes.io/change-cause", "managed_fields", "generation", "resource_version"]
    diff = RecursiveDictDiffer(old, new, True, True, ignore_keys)
    return diff.diffs
...
def statefulset_absent(name, namespace="default", **kwargs):
    """
    Ensures that the named stateful set is absent from the given namespace.
    ...
    stateful_set = __salt__["kubernetes.show_statefulset"](name, namespace, **kwargs)

    if stateful_set is None:
        ret["result"] = True if not __opts__["test"] else None
        ret["comment"] = "The stateful set does not exist"
        return ret

    if __opts__["test"]:
        ret["comment"] = "The stateful set is going to be deleted"
        ret["result"] = None
        return ret

    res = __salt__["kubernetes.delete_statefulset"](name, namespace, **kwargs)
    if res["code"] == 200:
        ret["result"] = True
        ret["changes"] = {
...
def statefulset_present(
    ...
    **kwargs):
    """
    Ensures that the named stateful set is present inside of the specified
    namespace with the given metadata and spec.
    If the stateful set exists it will be replaced.

    name
        The name of the stateful set.

    namespace
        The namespace holding the stateful set. The "default" one is going to be
        used unless a different one is specified.

    metadata
        The metadata of the stateful set object.

    spec
        The spec of the stateful set object.

    source
        A file containing the definition of the stateful set (metadata and
        spec) in the official kubernetes format.
    ...
    if stateful_set is None:
        if __opts__["test"]:
            ret["result"] = None
            ret["comment"] = "The stateful set is going to be created"
            return ret
        res = __salt__["kubernetes.create_statefulset"](name=name,
                                                       namespace=namespace,
                                                       metadata=metadata,
                                                       spec=spec,
                                                       source=source,
...
def ingress_present(
    ...
    **kwargs):
    ...
    namespace with the given metadata and spec.
    If the ingress exists it will be replaced.
    ...
    metadata
        The metadata of the ingress object.

    spec
        The spec of the ingress object.
    ...
    if ingress is None:
        if __opts__["test"]:
            ...
            return ret
        res = __salt__["kubernetes.create_ingress"](name=name,
                                                       namespace=namespace,
                                                       metadata=metadata,
                                                       spec=spec,
                                                       source=source,
